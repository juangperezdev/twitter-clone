      ...
      <main className="flex-1 max-w-[600px] border-r border-zinc-800 min-h-screen">
        ...
        <div className="flex border-b border-zinc-800">
          ...
          <Link href={`/${username}/following`} className="flex-1 flex justify-center hover:bg-zinc-900 transition cursor-pointer">
            <span className="py-4 text-zinc-500 font-medium text-sm">Siguiendo</span>
          </Link>
        </div>
        ...
        <div>
          {profileUser.followers.map(({ follower }) => {
            const amIFollowing = follower.followers.some(f => f.followerId === currentUserId)
            return (
              <div key={follower.id} className="p-4 hover:bg-zinc-950/40 transition flex items-start gap-3 border-b border-zinc-800">
                <Link href={`/${follower.username}`} className="w-10 h-10 bg-zinc-800 rounded-full shrink-0 overflow-hidden block">
                  <img src={getAvatar(follower.avatar, follower.username)} alt="" className="w-full h-full object-cover" />
                </Link>
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-center gap-2">
                    <Link href={`/${follower.username}`} className="truncate">
                      <p className="font-bold text-sm hover:underline truncate">{follower.name}</p>
                      <p className="text-zinc-500 text-sm truncate">@{follower.username}</p>
                    </Link>
                    {follower.id !== currentUserId && (
                      <FollowButton targetUserId={follower.id} isFollowing={amIFollowing} />
                    )}
                  </div>
                  {follower.bio && <p className="text-sm mt-1 text-zinc-300 line-clamp-2">{follower.bio}</p>}
                </div>
              </div>
            )
          })}
          {profileUser.followers.length === 0 && (
            <p className="text-zinc-500 text-center py-16 text-sm">Nadie sigue a @{username} todavía.</p>
          )}
        </div>
      </main>
